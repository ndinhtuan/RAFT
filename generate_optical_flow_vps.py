...
def demo(args):
    ...
    with torch.no_grad():
        ...
        for imfile1, imfile2 in zip(images[:-1], images[1:]):
            ...
            flow_name = imfile1.split("/")[-1].replace("newImg8bit", "opticalflow").replace("leftImg8bit", "opticalflow").replace("png", "npy")

            np.save(os.path.join(args.output_path, flow_name), flow_up)
